...
function matches(term: any, query: string) {
  ...
  const values = [term.transliteration, term.english_meaning, term.explanation, ...(term.aliases ?? [])]
  return values.some((value) => String(value ?? '').toLowerCase().includes(query))
}

export default async function TibetanPage({ searchParams }: { searchParams: Promise<{ q?: string; view?: string }> }) {
  ...
  const [{ data: rows }, bookmarkResult, progressResult] = await Promise.all([
    ...
    userId ? supabase.from('user_tibetan_bookmarks').select('term_id').eq('user_id', userId) : Promise.resolve({ data: [] } as any),
    userId ? supabase.from('user_tibetan_flashcard_progress').select('term_id, learning_state, review_count').eq('user_id', userId) : Promise.resolve({ data: [] } as any),
  ])
  ...
  const viewOptions: Array<[GlossaryView, string]> = [
    ['all', 'All terms'], ['saved', `Bookmarked · ${bookmarked.size}`], ['learning', `Learning · ${learningCount}`], ['learned', `Learned · ${learnedCount}`],
  ]
  ...
  return (
    <main className="container page">
      ...
      <p className="lead">Search Tibetan terminology by transliteration or English meaning, bookmark terms for later, and study them as flashcards.</p>
      ...
      {userId ? <section className="section card">
        ...
        <div style={{ height: 10, borderRadius: 999, overflow: 'hidden', background: 'var(--cream)', margin: '16px 0 10px' }}><div style={{ width: `${learnedPercent}%`, height: '100%', background: 'var(--sage-dark)' }} /></div>
        <div className="meta">{newCount} new · {learningCount} learning · {learnedCount} learned</div>
        <div className="actions" style={{ marginTop: 16 }}>{viewOptions.map(([key, label]) => <Link className={`button${activeView === key ? ' sage' : ''}`} key={key} href={viewHref(key)}>{label}</Link>)}</div>
      </section> : null}

      <section className="section card sage">
        <div className="eyebrow">Search</div>
        <form className="form-stack" method="get" action="/tibetan">
          <label>Find a term<input className="input" type="search" name="q" defaultValue={queryText} placeholder="Transliteration or English meaning…" /></label>
          {activeView !== 'all' ? <input type="hidden" name="view" value={activeView} /> : null}
          <div className="actions"><button className="button red" type="submit">Search glossary</button>{queryText ? <Link className="button" href={viewHref(activeView)}>Clear search</Link> : null}</div>
        </form>
      </section>
      ...
      <section className="section">
        ...
        {terms.length ? <div className="grid two">{terms.map((term: any) => {
          ...
          return <Link className="card" key={term.id} href={`/tibetan/${term.slug}`}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}><div className="eyebrow">{term.transliteration}</div>{userId ? <span className="pill">{bookmarked.has(term.id) ? '★ ' : ''}{state}</span> : null}</div>
        ...
        })}</div> : <div className="card"><h2 style={{ fontSize: 30 }}>{queryText ? 'No glossary terms match this search.' : 'No terms in this view yet.'}</h2><p className="meta">Try another filter or return to all glossary terms.</p></div>}
      </section>
