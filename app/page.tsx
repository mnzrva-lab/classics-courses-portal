...
export default async function HomePage() {
  ...
  const [sessionsResult, offeringsResult, claimsResult, perfectionResult] = await Promise.all([
    supabase
      .from('sessions')
      .select(`
        id, slug, code, title, session_type, starts_at, ends_at, zoom_url, offering_id,
    ...
    supabase
      ...
      .select(`
        id, slug, label, location, starts_on, ends_on, artwork_url, telegram_url, status,
        courses!inner(slug, title, canonical_number, status)
      `)
      .eq('status', 'published')
      .eq('courses.status', 'published')
    ...
    supabase
      .from('courses')
      .select('slug, title, subtitle, course_offerings(slug, label, status, sort_order)')
      .eq('status', 'published')
  ...
  const sessions = (sessionsResult.data ?? []).map((row: any) => {
    ...
    return {
      ...
      course_title: course?.title ?? 'Classics Course',
      course_label: courseLabel,
      course_slug: course?.slug,
      offering_label: offering?.label ?? null,
  ...
  if (userId && activeOffering) {
    const { data: activeSessions } = await supabase
      .from('sessions')
      .select('id, required_for_completion')
  ...
  const perfectionOffering = (perfection?.course_offerings ?? [])
    .filter((offering: any) => offering.status === 'published')
    .sort((a: any, b: any) => (a.sort_order ?? 0) - (b.sort_order ?? 0))[0]
  const perfectionHref = perfection && perfectionOffering
    ? `/courses/${perfection.slug}/${perfectionOffering.slug}`
    : '/other-programs'

  const milestones = offerings
    .filter((offering) => offering.id === currentOffering?.id || offering.starts_on >= today || (offering.ends_on && offering.ends_on >= today))
  ...
  const telegramHref = activeOffering?.telegram_url ?? offerings.find((offering) => offering.telegram_url)?.telegram_url ?? 'https://t.me/+muSMz-CJWB41ODkx'
    ...
    <main>
      <section className="hero home-v12-hero">
        <div className="container">
          ...
          <p>Live class access, recordings, Study Notes, Reference Transcripts, meditations, and course materials in one place.</p>
        </div>
      </section>

      <section className="container section home-next-section">
        <div className="home-time-note">Schedule times are shown in your local timezone by default.</div>
        <NextSessionCard sessions={sessions} />
      </section>

      <section className="container home-current-grid">
        {activeOffering && activeCourse && activeHref ? (
          <Link className="home-active-course" href={activeHref}>
            <div
              className={activeOffering.artwork_url ? 'home-active-artwork' : 'home-active-artwork placeholder'}
              style={activeOffering.artwork_url ? { backgroundImage: `url(${activeOffering.artwork_url})` } : undefined}
              aria-hidden="true"
            />
            <div className="home-active-copy">
              <div className="eyebrow">{progressPercent != null && progressPercent > 0 ? 'Continue learning' : 'Current course'}</div>
              <h2>{activeCourseLabel} · {activeOffering.label}</h2>
              <p>{activeCourse.title}{offeringRange(activeOffering.starts_on, activeOffering.ends_on) ? ` · ${offeringRange(activeOffering.starts_on, activeOffering.ends_on)}` : ''}{activeOffering.location ? ` · ${activeOffering.location}` : ''}</p>
              {progressPercent != null ? (
                <>
                  <div className="home-progress-row">
                    <div className="home-progress"><span style={{ width: `${Math.max(0, Math.min(100, progressPercent))}%` }} /></div>
                    <strong>{progressPercent}%</strong>
                  </div>
                  <div className="meta">Your progress</div>
                </>
              ) : null}
              <span className="button sage home-active-button">{progressPercent != null && progressPercent > 0 ? 'Continue course' : 'Open course'}</span>
            </div>
          </Link>
        ) : (
          <div className="card home-active-course-empty">
            <div className="eyebrow">Current course</div>
            <h2>Published teaching will appear here.</h2>
          </div>
        )}
      ...
      <section className="container section">
        <div className="section-head">
          <div>
            <h2>Now &amp; next</h2>
            <p>Important teaching milestones.</p>
          </div>
        </div>
        <div className="home-milestones">
          {milestones.length ? milestones.map((offering) => {
            const course = offering.courses as any
            const label = course?.canonical_number ? `Classics Course ${course.canonical_number}` : course?.title ?? 'Course'
            const href = `/courses/${course.slug}/${offering.slug}`
            return (
              <Link className="home-milestone-row" href={href} key={offering.id}>
                <strong className="home-milestone-date">{offeringRange(offering.starts_on, offering.ends_on) ?? 'Dates to be announced'}</strong>
                <div>
                  <h3>{label}{offering.label ? ` · ${offering.label}` : ''}</h3>
                  <p>{course?.title}</p>
                </div>
                <span className="home-open-pill">Open</span>
              </Link>
            )
          }) : <div className="card"><p className="meta">Upcoming published Course Offerings will appear here.</p></div>}
        </div>
      </section>
      ...
      <section className="container section">
        ...
        <div className="home-explore-grid">
          ...
          <Link className="card home-library-card" href={perfectionHref}>
      ...
      <section className="container home-telegram-end">
        <a className="button" href={telegramHref} target="_blank" rel="noreferrer">Telegram updates ↗</a>
      </section>
    </main>
  )
}
