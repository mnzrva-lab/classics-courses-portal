      ...
      <section className="section card">
        <form className="form-stack" action={submitTeachingSearch}>
          ...
          <div className="actions">
            <button className="button red" type="submit">Search</button>
            {userId ? <Link className="button" href={query ? `/my-notes?q=${encodeURIComponent(query)}` : '/my-notes'}>{query ? 'Search My Notes too' : 'Search My Notes'}</Link> : null}
          </div>
        </form>
        <p className="meta">Search history is not saved unless you turn it on in <Link href="/account">Privacy &amp; Data</Link>. My Notes searches only your private saved material.</p>
      </section>
