...
function safeFileName(name: string) {
  ...
  const stem = (dot > 0 ? name.slice(0, dot) : name).normalize('NFKD').replace(/[\u0300-\u036f]/g, '').replace(/[^A-Za-z0-9_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 80) || 'file'
...
async function nextSortOrder(supabase: Awaited<ReturnType<typeof createClient>>, offeringId: string) {
  const { data, error } = await supabase.from('materials').select('sort_order').eq('offering_id', offeringId).is('session_id', null).order('sort_order', { ascending: false }).limit(1).maybeSingle()
...
export async function addOfferingMaterial(offeringId: string, formData: FormData) {
  ...
  const { error } = await supabase.from('materials').insert({
    offering_id: offeringId, session_id: null, course_id: null,
...
export async function updateOfferingMaterial(offeringId: string, materialId: string, formData: FormData) {
  const supabase = await requireAdmin()
  const { data: existing, error: readError } = await supabase.from('materials').select('storage_path').eq('id', materialId).eq('offering_id', offeringId).is('session_id', null).single()
  ...
  }).eq('id', materialId).eq('offering_id', offeringId).is('session_id', null)
  if (error) throw new Error(error.message)
  revalidateOffering(offeringId)
  redirect(`/admin/offerings/${offeringId}?saved=offering-material`)
}
...
export async function deleteOfferingMaterial(offeringId: string, materialId: string) {
  const supabase = await requireAdmin()
  const { data: existing, error: readError } = await supabase.from('materials').select('storage_bucket, storage_path').eq('id', materialId).eq('offering_id', offeringId).is('session_id', null).single()
  if (readError) throw new Error(readError.message)
  ...
  const { error } = await supabase.from('materials').delete().eq('id', materialId).eq('offering_id', offeringId).is('session_id', null)
  if (error) throw new Error(error.message)
  revalidateOffering(offeringId)
  redirect(`/admin/offerings/${offeringId}?saved=offering-material`)
}
