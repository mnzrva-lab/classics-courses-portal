...
export default function PlaylistCsvImport({ offeringId }: { offeringId: string }) {
  ...
  async function handleFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const prepared = await preparePlaylistCsvImport(offeringId, raw as PlaylistCsvRow[])
  ...
  function setSession(rowKey: string, sessionId: string) {
    setRows((current) => current.map((row) => row.key === rowKey ? { ...row, sessionId, matchNote: sessionId ? 'Match selected manually.' : 'Choose the matching session.' } : row))
  }
  ...
  return (
    <details className="admin-playlist-import">
      <summary className="button">Import playlist CSV</summary>
      <div className="admin-playlist-import-panel">
        <p className="meta">Use the YouTube playlist CSV export. The importer matches Class numbers automatically and assigns unnumbered meditations in teaching order. You can correct every match before applying.</p>
