...
export async function preparePlaylistCsvImport(offeringId: string, rows: PlaylistCsvRow[]) {
  ...
  for (const row of chronologicalRows) {
    ...
    if (explicitClass != null) {
      ...
      preparedByKey.set(row.key, { ...row, sessionId: target?.id ?? '', matchNote: target ? `Matched by Class ${explicitClass}.` : `No C${explicitClass} session exists in this Course Offering.` })
      continue
    }
    ...
    if (explicitMeditation != null) {
      ...
      preparedByKey.set(row.key, { ...row, sessionId: target?.id ?? '', matchNote: target ? `Matched by Meditation ${explicitMeditation}.` : `No M${explicitMeditation} session exists in this Course Offering.` })
      continue
    }
    if (/\bmeditation\b/i.test(row.videoTitle)) {
      const target = meditationSessions.find((session) => !usedMeditations.has(session.id))
      if (target) usedMeditations.add(target.id)
      preparedByKey.set(row.key, { ...row, sessionId: target?.id ?? '', matchNote: target ? `Matched to ${target.code ?? target.title} by playlist teaching order.` : 'No unmatched meditation session remains.' })
      continue
    }
    preparedByKey.set(row.key, { ...row, sessionId: '', matchNote: 'Choose the matching session.' })
  }

  return { sessions, rows: rows.map((row) => preparedByKey.get(row.key) ?? { ...row, sessionId: '', matchNote: 'Choose the matching session.' }) }
}

export async function applyPlaylistCsvImport(offeringId: string, rows: Array<{ sessionId: string; videoUrl: string }>, playlist: { url: string; title: string }) {
  const supabase = await requireAdmin()
  const { data: offering, error: offeringError } = await supabase.from('course_offerings').select('id, slug, courses(slug)').eq('id', offeringId).single()
  if (offeringError || !offering) throw new Error(offeringError?.message ?? 'Course Offering not found.')
  const { data: sessionRows, error: sessionError } = await supabase.from('sessions').select('id').eq('offering_id', offeringId)
  ...
  if (playlistUrl) {
    const { data: existing, error: existingError } = await supabase.from('materials').select('id').eq('offering_id', offeringId).is('session_id', null).eq('material_type', 'video').ilike('title', '%playlist%').limit(1).maybeSingle()
    if (existingError) throw new Error(existingError.message)
    if (existing) {
      const { error } = await supabase.from('materials').update({ title: 'Course recordings playlist', url: playlistUrl, status: 'published', updated_at: new Date().toISOString() }).eq('id', existing.id)
      if (error) throw new Error(error.message)
    } else {
      const { data: highest, error: highestError } = await supabase.from('materials').select('sort_order').eq('offering_id', offeringId).is('session_id', null).order('sort_order', { ascending: false }).limit(1).maybeSingle()
      if (highestError) throw new Error(highestError.message)
      const { error } = await supabase.from('materials').insert({ offering_id: offeringId, session_id: null, course_id: null, material_type: 'video', title: 'Course recordings playlist', url: playlistUrl, mime_type: 'text/html', status: 'published', sort_order: (highest?.sort_order ?? -1) + 1 })
      if (error) throw new Error(error.message)
    }
    playlistSaved = true
  }
