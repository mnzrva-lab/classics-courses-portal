...
function parseTranscript(rawText: string, transcriptId: string, sessionId: string) {
  ...
  for (const block of blocks) {
    ...
    if (imageMatch) {
      ...
      if (!storagePath.startsWith(`transcripts/${sessionId}/`) || storagePath.includes('..')) throw new Error('Transcript image upload path is invalid.')
      assets.push({ transcript_id: transcriptId, after_paragraph_sort_order: paragraphOrder - 1, asset_type: 'image', storage_bucket: TRANSCRIPT_ASSETS_BUCKET, storage_path: storagePath, mime_type: imageMatch[2].trim() || null, alt_text: null, caption: null, sort_order: assetOrder++ })
      continue
    }
...
export async function importTranscriptDraft(offeringId: string, sessionId: string, sourceFileName: string, rawText: string) {
  const supabase = await requireAdmin()
  const { data: session, error: sessionError } = await supabase.from('sessions').select('id, offering_id').eq('id', sessionId).eq('offering_id', offeringId).maybeSingle()
  if (sessionError) throw new Error(sessionError.message)
  if (!session) throw new Error('The selected session does not belong to this Course Offering.')

  const { data: existing, error: existingError } = await supabase.from('transcripts').select('id, status').eq('session_id', sessionId).eq('language_code', 'en').maybeSingle()
  if (existingError) throw new Error(existingError.message)
  if (existing) return { ok: false, skipped: true, message: `Skipped because this session already has a ${existing.status} transcript.` }
  ...
  const paragraphPayload = paragraphs.map((paragraph) => ({ id: randomUUID(), section_id: paragraph.section_id, speaker: paragraph.speaker, body: paragraph.body, start_seconds: paragraph.start_seconds, sort_order: paragraph.sort_order }))
  ...
  return { ok: true, skipped: false, message: `Imported as Published: ${paragraphs.length} paragraphs${assets.length ? `, ${assets.length} image${assets.length === 1 ? '' : 's'}` : ''}.` }
}
