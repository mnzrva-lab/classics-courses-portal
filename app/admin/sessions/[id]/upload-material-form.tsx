...
export default function UploadMaterialForm({ sessionId }: { sessionId: string }) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const storagePath = `sessions/${sessionId}/${crypto.randomUUID()}-${safeFileName(file.name)}`
  ...
  return (
    <form className="form-stack" onSubmit={handleSubmit}>
      ...
      <p className="meta">Direct upload is currently optimized for files up to 6 MB. Larger PDFs, slide decks, audio, or video can still be added with a stable external link below.</p>
